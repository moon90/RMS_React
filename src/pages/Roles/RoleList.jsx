...
export default function RoleList() {
  ...
  const handleToggleStatus = async (role) => {
    ...
    try {
      const response = await toggleRoleStatus(role.roleID);
      if (response.data.isSuccess) {
        toast.success(`Role '${role.roleName}' status updated.`);
        fetchRoles();
      }
    } catch (error) {
      toast.error('Authority status update failed.');
    }
  };
  ...
  const handleDelete = (id) => {
    ...
    toast(({ closeToast }) => (
      <div className="p-1">
        <p className="text-sm font-bold text-gray-800 mb-3">Purge this security role permanently?</p>
          ...
          <button
            onClick={async () => {
              try {
                ...
                toast.error('Cannot purge role: Active dependencies found');
              }
              closeToast();
            }}
    ...
    <div className="container mx-auto p-6 animate-fade-in max-w-7xl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
        <div>
          ...
          <p className="text-gray-500 mt-1 font-medium italic">Manage system roles and permissions</p>
        </div>
        
        {canCreateRole && (
          <button
            onClick={() => { setSelectedRole(null); setIsEditModalOpen(true); }}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100">
                <th className="px-8 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest cursor-pointer" onClick={() => handleSort('RoleName')}>Role Details</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading ? (
                ...
                roles.map((role) => (
                  <tr key={role.roleID} className="hover:bg-gray-50/50 transition-all group">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <div className="relative">
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black shadow-inner border-2 border-white ring-4 ring-gray-50 group-hover:ring-blue-50 transition-all ${role.status ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-400'}`}>
                            <FaFingerprint />
                          </div>
                          <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white shadow-sm ${role.status ? 'bg-green-500' : 'bg-red-400'}`}></div>
                        </div>
                        <div className="flex flex-col">
                          <span className="font-black text-gray-800 text-sm tracking-tight uppercase">{role.roleName}</span>
                          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">ID: {role.roleID}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      <div className="flex flex-col max-w-[300px]">
                        <span className="text-xs font-bold text-gray-500 italic flex items-center gap-2">
                          <FaFileAlt className="text-gray-200" /> {role.description || 'No system definition provided.'}
                        </span>
                      </div>
                    </td>
                    <td className="px-8 py-6 text-center">
                      <button 
                        ...
                        className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all border shadow-sm ${
                          role.status ? 'bg-green-50 text-green-700 border-green-100 hover:bg-green-100' : 'bg-red-50 text-red-700 border-red-100 hover:bg-red-100'
                        }`}
                      >
                        {role.status ? <><FaCheckCircle /> Active</> : <><FaTimesCircle /> Inactive</>}
                      </button>
                    </td>
                    <td className="px-8 py-6 text-right">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                        {canUpdateRole && (
                          <button onClick={() => { setSelectedRole(role); setIsEditModalOpen(true); }} className="p-3 bg-white border border-gray-100 rounded-xl text-blue-600 hover:shadow-xl transition-all hover:scale-110 shadow-sm"><FaEdit /></button>
                        )}
                        {canDeleteRole && (
                          <button onClick={() => handleDelete(role.roleID)} className="p-3 bg-white border border-gray-100 rounded-xl text-red-600 hover:shadow-xl transition-all hover:scale-110 shadow-sm"><FaTrashAlt /></button>
                        )}
                      </div>
                    </td>
                  </tr>
                ...
                <tr>
                  <td colSpan="4" className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-4 text-gray-300">
                      <FaShieldAlt size={60} />
                      <p className="text-xl font-black uppercase tracking-widest text-gray-300">No Authority Nodes Found</p>
                    </div>
                  </td>
                </tr>
              )}
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fade-in">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-scale-in">
            <div className="p-10 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <div>
                ...
                <p className="text-xs font-bold text-gray-400 mt-1 uppercase tracking-widest">
                  {selectedRole ? `ID: ${selectedRole.roleID}` : 'Role Management'}
                </p>
              </div>
            ...
            <div className="p-10 overflow-y-auto flex-1 custom-scrollbar text-center">
              <RoleAdd 
                isEdit={!!selectedRole} 
                roleData={selectedRole} 
                onClose={() => setIsEditModalOpen(false)} 
                onSave={() => { fetchRoles(); setIsEditModalOpen(false); }}
                showTitle={false}
              />
            </div>
          </div>
