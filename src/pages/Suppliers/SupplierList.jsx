...
export default function SupplierList() {
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      ...
      if (response.data && response.data.isSuccess) {
        ...
        toast.error('Failed to synchronize supplier registry.');
        setSuppliers([]);
        setTotalSuppliers(0);
      }
    } catch (error) {
  ...
  const handleToggleStatus = async (supplier) => {
    if (!canUpdate) return;
    try {
      const response = await toggleSupplierStatus(supplier.id, !supplier.status);
      if (response.data.isSuccess) {
        toast.success(`Supplier '${supplier.supplierName}' status updated.`);
        fetchSuppliers();
      }
    } catch (error) {
  ...
  const handleDelete = (id) => {
    ...
    toast(({ closeToast }) => (
      <div className="p-1">
        <p className="text-sm font-bold text-gray-800 mb-3">Purge this supplier permanently?</p>
          ...
          <button
            onClick={async () => {
              try {
                ...
                if (response.data.isSuccess) {
                  toast.success('Supplier purged successfully');
                  fetchSuppliers();
                } else {
                  toast.error(response.data.message || 'Purge rejected by system');
                }
              } catch (err) {
                toast.error('Cannot purge supplier: Active dependencies found');
              }
              closeToast();
            }}
    ...
    <div className="container mx-auto p-6 animate-fade-in max-w-7xl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
        <div>
          ...
          <p className="text-gray-500 mt-1 font-medium italic">Manage supply chain partners and logistics</p>
        </div>
        
        {canCreate && (
          <button
            onClick={() => { setSelectedSupplier(null); setIsEditModalOpen(true); }}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100">
                <th className="px-8 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest cursor-pointer" onClick={() => handleSort('SupplierName')}>Supplier Details</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading ? (
                ...
                suppliers.map((supplier) => (
                  <tr key={supplier.id} className="hover:bg-gray-50/50 transition-all group">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <div className="relative">
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black shadow-inner border-2 border-white ring-4 ring-gray-50 group-hover:ring-blue-50 transition-all ${supplier.status ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-400'}`}>
                            <FaTruckLoading />
                          </div>
                          <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white shadow-sm ${supplier.status ? 'bg-green-500' : 'bg-red-400'}`}></div>
                        </div>
                        <div className="flex flex-col">
                          <span className="font-black text-gray-800 text-sm tracking-tight uppercase">{supplier.supplierName}</span>
                          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">ID: {supplier.id}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-1 text-xs font-bold text-gray-600">
                        <span className="flex items-center gap-2"><FaUserTie className="text-gray-300" /> {supplier.contactPerson}</span>
                        <span className="flex items-center gap-2 text-gray-400 font-medium"><FaPhone className="text-gray-200" /> {supplier.phone}</span>
                        <span className="flex items-center gap-2 text-gray-400 font-medium lowercase italic"><FaEnvelope className="text-gray-200" /> {supplier.email}</span>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      <button 
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all border shadow-sm ${
                          supplier.status ? 'bg-green-50 text-green-700 border-green-100 hover:bg-green-100' : 'bg-red-50 text-red-700 border-red-100 hover:bg-red-100'
                        }`}
                      >
                        {supplier.status ? <><FaCheckCircle /> Active</> : <><FaTimesCircle /> Inactive</>}
                      </button>
                    </td>
                    <td className="px-8 py-6 text-right">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                        {canUpdate && (
                          <button onClick={() => { setSelectedSupplier(supplier); setIsEditModalOpen(true); }} className="p-3 bg-white border border-gray-100 rounded-xl text-blue-600 hover:shadow-xl transition-all hover:scale-110 shadow-sm"><FaEdit /></button>
                        )}
                        {canDelete && (
                          <button onClick={() => handleDelete(supplier.id)} className="p-3 bg-white border border-gray-100 rounded-xl text-red-600 hover:shadow-xl transition-all hover:scale-110 shadow-sm"><FaTrashAlt /></button>
                        )}
                      </div>
                    </td>
                  </tr>
                ...
                <tr>
                  <td colSpan="4" className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-4 text-gray-300">
                      ...
                      <p className="text-xl font-black uppercase tracking-widest text-gray-300">No Suppliers Found</p>
                    </div>
                  </td>
                </tr>
              )}
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-scale-in">
            <div className="p-10 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <div>
                ...
                <p className="text-xs font-bold text-gray-400 mt-1 uppercase tracking-widest">
                  {selectedSupplier ? `ID: ${selectedSupplier.id}` : 'Supplier Management'}
                </p>
              </div>
            ...
            <div className="p-10 overflow-y-auto flex-1 custom-scrollbar">
              <SupplierAdd 
                ...
                onSave={() => { fetchSuppliers(); setIsEditModalOpen(false); }}
                showTitle={false}
              />
            </div>
          </div>
