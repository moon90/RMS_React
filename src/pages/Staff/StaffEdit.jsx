...
const StaffEdit = () => {
    ...
    const fetchStaff = async () => {
      try {
        ...
        if (response.data.isSuccess) {
          ...
          setStaffPhone(staff.staffPhone || '');
          setStaffRole(staff.staffRole || '');
          setStatus(staff.status);
        } else {
          toast.error(response.data.message || 'Failed to fetch staff member.');
        }
      } catch (err) {
  ...
  if (loading) {
    return <div className="p-3 max-w-4xl mx-auto">Loading staff member...</div>;
  }
